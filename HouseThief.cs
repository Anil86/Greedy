...
namespace Greedy
{
    ...
    {
        private (List<(int, int)> values, int total) MaximizeValue(int[] houses)
        {
            int index = 0;
            List<(int value, int i)> stolenValues = new List<(int, int)>(houses.Length / 2 + 1);
        ...
        internal static void Work()
        {
            ...
            foreach ((int value, var _) in stolenValues) Write($"{value:C0}  ");
            WriteLine($"\nTotal: {total}");
        }
    }
}